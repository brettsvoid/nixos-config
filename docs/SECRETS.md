...
│   └── inputs.secrets = git+ssh://...          ├── nomad-token.age
│                                              ├── sentry-mcp-token.age
├── modules/                                  ├── grafana-mcp-token.age
│   ├── flake/agenix.nix                      ├── github-ssh-key.age
│   └── system/{nixos,darwin}/                ├── syncthing-cert.age
...
3. **Register the key in the private repo's `secrets.nix`**:
   ...
   in
   {
     "nomad-token.age".publicKeys = all-hosts;
     # ...
   }
   ```
...
- **Filename leak**: `wg0PrivateKey.age` reveals "I run wireguard." `syncthing-cert.age` reveals "I run syncthing." For a personal threat model with low value-of-data, this is fine. For someone whose full digital life is on these machines, hiding service names is cheap insurance.
